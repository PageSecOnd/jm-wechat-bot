...
> 项目/仓库名为 `jm-wechat-bot`；为兼容既有部署，Python 包、CLI 命令和环境变量前缀继续使用 `jmwxbot` / `JMWXBOT_`。

> 仅用于你有权访问、下载和传输的内容；请遵守内容源条款、版权要求及当地法律。

## v1.0.0 功能

> **v1.0.0 认证升级提示**：认证会话固定绑定到实际登录成功的 JM API 域名，并保存完整会话 Cookie。v0.6.1 及更早生成的 AVS-only profile 缺少认证域名，升级后请每位已登录用户重新执行一次 `/login 用户名 密码`。
...
v0.6.0 新增只读管理控制台。默认未设置管理 token 时完全禁用。
